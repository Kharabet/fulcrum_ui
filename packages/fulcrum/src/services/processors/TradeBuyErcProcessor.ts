...
export class TradeBuyErcProcessor {
  public run = async (task: RequestTask, account: string, skipGas: boolean) => {
    ...
    if (approvePromise || skipGas) {
      ...
      const gasAmount = await tokenContract.mintWithToken.estimateGasAsync(account, assetErc20Address, amountInBaseUnits, { from: account, gas: FulcrumProvider.Instance.gasLimit });
