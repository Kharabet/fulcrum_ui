...
export class AssetDropdown extends Component<IAssetDropdownProps, IAssetDropdownState> {
  ...
  componentDidUpdate(
    prevProps: Readonly<IAssetDropdownProps>,
    prevState: Readonly<IAssetDropdownState>,
    snapshot?: any
  ): void {
    if (prevState.isOpened !== this.state.isOpened) {
      if (document.querySelector('.asset-dropdown__wrapper')) {
        const assetDropdownSelector = document.querySelector(
          '.asset-dropdown__wrapper'
        ) as HTMLElement
  ...
  tokenItems() {
    return this.props.assets
      .filter((asset) => asset !== this.props.selectedAsset)
      .map((e) => {
  ...
  public render() {
    ...
    return (
      <div className="asset-dropdown-container">
        <div
          className={`asset-dropdown-button ${this.state.isOpened ? 'opened' : 'closed'} ${
