...
export class ProgressBar extends Component<IProgressBarProps> {
      ...
      <div className="progress-bar-container">
        <div className="progress-bar">
          <div className="progress-bar__info-container">
            ...
            <div className="progress-bar__info">
              <div className="progress-bar__title">
                {this.props.requestTask.steps[this.props.requestTask.stepCurrent - 1]}
              </div>
          ...
          <div className="progress-bar__actions-container">
            ...
            <button
              className="progress-bar__btn progress-bar__btn--more"
              onClick={this.props.onViewMore}>
              View more
            </button>
          </div>
        </div>
  ...
  public renderTaskFailedStateActions = (): ReactNode => {
    ...
    return this.props.requestTask.status === RequestStatus.FAILED ||
      this.props.requestTask.status === RequestStatus.FAILED_SKIPGAS ? (
      <React.Fragment>
        {errorMsg ? (
          <React.Fragment>
            <div className="progress-bar__error-message">{errorMsg}</div>
          </React.Fragment>
        ) : (
          ``
        )}
        {forceRetry ? (
          <button
            className="progress-bar__btn progress-bar__btn--try-again"
