...
export class LendTokenSelectorItem extends Component<
  ILendTokenSelectorItemProps,
  ILendTokenSelectorItemState
> {
  ...
  private async derivedUpdate() {
    ...
    if (profit && profit.lt(0)) profit = new BigNumber(0)

    const balanceOfUser = await FulcrumProvider.Instance.getITokenAssetBalanceOfUser(
      this.props.asset
    )
  ...
  public async componentDidMount() {
    ...
    const task = await TasksQueue.Instance.getTasksList().find(
      (t) => t.request instanceof LendRequest && t.request.asset === this.props.asset
    )
    ...
    this._refreshInterval = window.setInterval(
      this.derivedUpdate.bind(this),
      this._refreshProfitTimerMillisec
    )
  }
    ...
    return (
      <div
        className={`token-selector-item ${
          this.state.balanceOfUser.eq(0) ? '' : 'token-selector-item_active'
        } ${this.state.isLoadingTransaction ? 'loading-transaction' : ''}`}>
        ...
        {this.state.isLoadingTransaction && this.state.request ? (
          ...
          <React.Fragment>
            <div
              ...
              style={{ marginTop: this.state.profit === null ? `1.5rem` : undefined }}>
                  ...
                  <React.Fragment>
                      ...
                      <div className="token-selector-item__profit-container token-selector-item__balance-container">
                        ...
                        {!this.state.isLoading ? (
                          <div
                            ...
                            {this.state.balanceOfUser.toFixed(2)}
                          </div>
                        ) : (
                          <div className="token-selector-item__interest-rate-value">
                            <Preloader width="74px" />
                    ...
                    <div className="token-selector-item__profit-container">
                      ...
                      <ProfitTicker
                        ...
                        secondDiff={this.state.tickerSecondDiff}
