...
export class TradeTxLoaderStep extends Component<ITradeTxLoaderStepProps, ITradeTxLoaderStepState> {
  ...
  public componentDidUpdate(
    prevProps: Readonly<ITradeTxLoaderStepProps>,
    prevState: Readonly<ITradeTxLoaderStepState>
  ): void {
    const div = this.stepDiv.current
    if (!div) return
    if (
      ...
      <React.Fragment>
        {this.state.requestTask && this.state.requestTask.txHash ? (
          <a
            ...
            <div
              ref={this.stepDiv}
              className={`trade-transaction-step ${this.state.title.isWarning ? 'warning' : ''}`}>
