...
export class TradeTokenKey {
  ...
  public toString(): string {
    const unitOfAccountPrefix =
      this.unitOfAccount === Asset.DAI ? 'd' : this.unitOfAccount === Asset.SAI ? 's' : 'u' // DAI, SAI and USDC
  ...
  public static fromString(value: string): TradeTokenKey | null {
    ...
    if (matches && matches.length > 0) {
      if (matches[0] === value) {
        const unitOfAccount =
          matches[1].toString().toLowerCase() === 'd'
            ? Asset.DAI
            : matches[1].toString().toLowerCase() === 's'
            ? Asset.SAI
            : Asset.USDC
        const positionType =
          matches[2].toString().toUpperCase() === 'L' ? PositionType.LONG : PositionType.SHORT
