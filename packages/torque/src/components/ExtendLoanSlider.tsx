...
import { ExtendLoanSliderHandle } from "./ExtendLoanSliderHandle";
import { ExtendLoanSliderRail } from "./ExtendLoanSliderRail";

export interface IExtendLoanSliderProps {
  readonly: boolean;
  minValue: number;
  maxValue: number;
  value: number;
...
export class ExtendLoanSlider extends Component<IExtendLoanSliderProps> {
  public render() {
    return (
      <Slider
      ...
      >
        <Rail>{(railObject: RailObject) => <ExtendLoanSliderRail getRailProps={railObject.getRailProps} />}</Rail>
        <Handles>
          {(handlesObject: HandlesObject) => (
            <div className="slider-handles">
              {handlesObject.handles.map(handle => (
                <ExtendLoanSliderHandle
                  key={handle.id}
                  handle={handle}
                  domain={[this.props.minValue, this.props.maxValue]}
