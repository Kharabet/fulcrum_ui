...
export class TasksListItemDetails extends Component<ITasksListItemDetailsProps> {
    ...
    const stepStateClassName =
      index + 1 === this.props.task.stepCurrent
        ? this.props.task.status === RequestStatus.FAILED ||
          this.props.task.status === RequestStatus.FAILED_SKIPGAS
  ...
  public renderStepProgressIndicator = (index: number): ReactNode => {
    ...
    return tx && TorqueProvider.Instance.web3ProviderSettings ? (
      <a
        ...
        href={`${TorqueProvider.Instance.web3ProviderSettings!.etherscanURL}tx/${tx}`}
        target="_blank"
  ...
  public renderTaskFailedStateActions = (index: number): ReactNode => {
    ...
    if (this.props.task.error) {
      ...
      if (errorMsg) {
        if (
          ...
          errorMsg =
            'The transaction seems like it will fail. You can submit the transaction anyway, or cancel.'
          forceRetry = true
    ...
    return (this.props.task.status === RequestStatus.FAILED ||
      this.props.task.status === RequestStatus.FAILED_SKIPGAS) &&
      index + 1 === this.props.task.stepCurrent ? (
      <React.Fragment>
        {errorMsg ? (
          <React.Fragment>
            <div className="task-list-item-details__step-title--failed-txn">{errorMsg}</div>
            {tx && TorqueProvider.Instance.web3ProviderSettings ? (
              <a
  ...
  private onTaskRetry = async () => {
    await TorqueProvider.Instance.onTaskRetry(this.props.task, false)
  }

  private onForceRetry = async () => {
    await TorqueProvider.Instance.onTaskRetry(this.props.task, true)
  }

  private onTaskCancel = async () => {
    await TorqueProvider.Instance.onTaskCancel(this.props.task)
  }
}
