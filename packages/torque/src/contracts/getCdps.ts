...
export class GetCdpsContract extends BaseContract {
  public getCdpsAsc = {
    async callAsync(
      ...
      const encodedData = self._strictEncodeArguments('getCdpsAsc(address,address)', [manager, guy])
  ...
  public getCdpsDesc = {
    async callAsync(
      ...
      const self = (this as any) as GetCdpsContract
      const encodedData = self._strictEncodeArguments('getCdpsDesc(address,address)', [
        manager,
        guy
      ])
      const callDataWithDefaults = await BaseContract._applyDefaultsToTxDataAsync(
