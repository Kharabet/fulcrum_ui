...
export class LiquidationProcessor {
    public run = async (task: RequestTask, account: string, skipGas: boolean) => {
        if (!(ExplorerProvider.Instance.contractsSource && ExplorerProvider.Instance.contractsSource.canWrite)) {
        ...
        if (!isETHLoanToken) {
            ...
            if (assetErc20Address) {
                tokenErc20Contract = await ExplorerProvider.Instance.contractsSource.getErc20Contract(assetErc20Address);
            } else {
            ...
            if (taskRequest.closeAmount.gt(erc20allowance)) {
                const approveHash = await tokenErc20Contract!.approve.sendTransactionAsync(iBZxContract.address, ExplorerProvider.Instance.getLargeApprovalAmount(taskRequest.loanToken, taskRequest.closeAmount), { from: account });
        ...
        const sendAmountForValue = isETHLoanToken ?
        ...
        try {
            ...
            gasAmountBN = new BigNumber(gasAmount).multipliedBy(ExplorerProvider.Instance.gasBufferCoeff).integerValue(BigNumber.ROUND_UP);
        } catch (e) {
            console.log(e);
            throw e;
        }
