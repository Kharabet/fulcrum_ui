...
window.addEventListener('load', function() {
  ...
  gainRange.addEventListener('input', function() {
    ...
    beforeGain.style.left =
      'calc(50% + ' + this.value / 2 + '% - 33px - (20px *' + this.value / 100 + '))' //20 - half of width thumb, 33 - half of with before-gain
    if (this.value < 0) {
  ...
  spinner.querySelector('.up').addEventListener(
    'click',
    function(e) {
      gainRange.value++
      onSpinnerClick()
  ...
  spinner.querySelector('.down').addEventListener(
    'click',
    function(e) {
      gainRange.value--
      onSpinnerClick()
