...
function renderAPR() {
  ...
  aprComponentElements.forEach(function(item) {
    ...
    if (apr[token]) item.querySelector('.apr-value').textContent = formatUsdPrice(apr[token])
  })

  var activeCalcWidgetToken = document.querySelector('.coin-calc.active').dataset.token
  document.querySelector('.item-earn.fulcrum .apr-value').textContent = formatUsdPrice(
    apr[activeCalcWidgetToken]
  )
  ...
  if (window.aprRenderer) clearInterval(window.aprRenderer)

  ///set data polling to update widgets every 60 secs
  if (!window.aprPolling) window.aprPolling = setInterval(updateData, 1000 * 60)
}
...
function changePositionBorderThumb(range, current) {
  leftRangeQuantity.style.left =
    'calc(' +
    (current.value / range.max) * 100 +
    '% - 12px - (16px *' +
    (current.value - range.max / 2) / range.max +
    '))' //12 - half of width thumb with border, 16 - width thumb without border
  rightRangeQuantity.style.left =
    'calc(' +
    (current.value / range.max) * 100 +
    '% - 12px + 20px - (16px *' +
    (current.value - range.max / 2) / range.max +
    '))' //12 - half of width thumb with border, 16 - width thumb without border
  trackRangeQuantity.style.width =
    'calc(' +
    (current.value / range.max) * 100 +
    '% - 12px - (16px *' +
    (current.value - range.max / 2) / range.max +
...
window.addEventListener('load', function() {
  ...
  quantityInput.oninput = function(e) {
    if (!e.currentTarget.value) quantityRange.value = 0
