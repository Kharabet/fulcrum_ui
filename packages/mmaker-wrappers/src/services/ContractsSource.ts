...
import { erc20Contract } from "../wrappers/erc20";
import { iTokenContract } from "../wrappers/iTokenContract";
import { marketMakerProvisionContract } from "../wrappers/marketMakerProvisionContract";
import { oracleContract } from "../wrappers/oracle";
import { pTokenContract } from "../wrappers/pTokenContract";
import { TokenizedRegistryContract } from "../wrappers/TokenizedRegistryContract";

import * as erc20Json from "./../../assets/artifacts/erc20.json";
import * as iTokenJson from "./../../assets/artifacts/iToken.json";
import * as marketMakerProvisionJson from "./../../assets/artifacts/marketMakerProvision.json";
import * as pTokenJson from "./../../assets/artifacts/pToken.json";
import * as oracleJson from "./../../assets/artifacts/oracle.json";
import * as TokenizedRegistryJson from "./../../assets/artifacts/TokenizedRegistry.json";

import { TokenList } from "../../assets/artifacts/tokenList";
...
export class ContractsSource {
  ...
  public async Init(network: string) {
    if (network === "mainnet") {
      ...
      this.tokenizedRegistryContract = new TokenizedRegistryContract(
        TokenizedRegistryJson.abi as any,
  ...
  private getMarketMakerProvisionAddress(): string {
    let address: string = "";
    switch (this.networkId) {
      case 42:
        address = "0xE41B37de302c41235b5aB5249Cdb965DCc81f573";
        break;
    }
  ...
  private async getErc20ContractRaw(addressErc20: string): Promise<erc20Contract> {
    return new erc20Contract(erc20Json.abi as any, addressErc20.toLowerCase(), this.provider);
  }
  ...
  private async getITokenContractRaw(asset: Asset): Promise<iTokenContract | null> {
    ...
    return tokenContractInfo ? new iTokenContract(iTokenJson.abi as any, tokenContractInfo.token, this.provider) : null;
  }
  ...
  private async getPTokenContractRaw(key: TradeTokenKey): Promise<pTokenContract | null> {
    ...
    return tokenContractInfo ? new pTokenContract(pTokenJson.abi as any, tokenContractInfo.token, this.provider) : null;
  }

  private async getOracleContractRaw(): Promise<oracleContract> {
    return new oracleContract(
      oracleJson.abi as any,
      this.getOracleAddress().toLowerCase(),
      this.provider
    );
  }

  private async getMarketMakerProvisionContractRaw(): Promise<marketMakerProvisionContract> {
    return new marketMakerProvisionContract(
      marketMakerProvisionJson.abi as any,
      this.getMarketMakerProvisionAddress().toLowerCase(),
      this.provider
    );
  }
  ...
  public getMarketMakerProvisionContract = _.memoize(this.getMarketMakerProvisionContractRaw);
}
