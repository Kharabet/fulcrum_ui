...
> {
    ...
    const functionSignature = 'amountInEth(address,uint256)';
...
> {
    const self = this as any as oracleContract;
      ...
      assert.isBigNumber('destAmount', destAmount);
      assert.isBigNumber('maxSlippage', maxSlippage);
    const functionSignature = 'checkPriceDisagreement(address,address,uint256,uint256,uint256)';
...
> {
    ...
    const functionSignature = 'decimals(address)';
...
> {
    const self = this as any as oracleContract;
      ...
      assert.isBigNumber('loanAmount', loanAmount);
      assert.isBigNumber('collateralAmount', collateralAmount);
    const functionSignature = 'getCurrentMarginAndCollateralSize(address,address,uint256,uint256)';
...
> {
    const self = this as any as oracleContract;
      assert.isString('payToken', payToken);
    const functionSignature = 'getFastGasPrice(address)';
...
> {
    const self = this as any as oracleContract;
      ...
      assert.isBigNumber('loanAmount', loanAmount);
      assert.isBigNumber('collateralAmount', collateralAmount);
      assert.isBigNumber('margin', margin);
    const functionSignature = 'getMaxDrawdown(address,address,uint256,uint256,uint256)';
...
> {
    const self = this as any as oracleContract;
    const functionSignature = 'globalPricingPaused()';
...
> {
    const self = this as any as oracleContract;
      assert.isString('index_0', index_0);
    const functionSignature = 'pricesFeeds(address)';
...
> {
    const self = this as any as oracleContract;
      assert.isString('sourceToken', sourceToken);
      assert.isString('destToken', destToken);
    const functionSignature = 'queryPrecision(address,address)';
...
> {
    const self = this as any as oracleContract;
      ...
      assert.isString('destToken', destToken);
      assert.isBigNumber('sourceAmount', sourceAmount);
    const functionSignature = 'queryReturn(address,address,uint256)';
...
> {
    ...
    const functionSignature = 'setDecimals(address[])';
...
> {
    const self = this as any as oracleContract;
      assert.isBoolean('isPaused', isPaused);
    const functionSignature = 'setGlobalPricingPaused(bool)';
    ...
    return {
      ...
      getABIEncodedTransactionData(): string {
        return self._strictEncodeArguments(functionSignature, [isPaused
      ]);
      },
    }
  };
...
> {
    const self = this as any as oracleContract;
      assert.isArray('tokens', tokens);
      assert.isArray('feeds', feeds);
    const functionSignature = 'setPriceFeed(address[],address[])';
...
> {
    const self = this as any as oracleContract;
      ...
      assert.isBigNumber('loanAmount', loanAmount);
      assert.isBigNumber('collateralAmount', collateralAmount);
      assert.isBigNumber('maintenanceMargin', maintenanceMargin);
    const functionSignature = 'shouldLiquidate(address,address,uint256,uint256,uint256)';
...
> {
  ...
  constructor(
    ...
    super('oracle', oracleContract.ABI(), address, supportedProvider, txDefaults, logDecodeDependencies, deployedBytecode);
