...
public static contractName = 'DAppHelper';
...
public static async deployFrom0xArtifactAsync(
    ...
    return DAppHelperContract.deployAsync(bytecode, abi, provider, txDefaults, logDecodeDependenciesAbiOnly, _bZxProtocol,
_legacyVault
);
  }
  ...
  public static async deployWithLibrariesFrom0xArtifactAsync(
    ...
    const libraryAddresses = await DAppHelperContract._deployLibrariesAsync(
      artifact,
      libraryArtifacts,
...
_legacyVault
    ...
    const txData = deployInfo.encode(bytecode, [_bZxProtocol,
_legacyVault
]);
    ...
    logUtils.log(`DAppHelper successfully deployed at ${txReceipt.contractAddress}`);
    const contractInstance = new DAppHelperContract(txReceipt.contractAddress as string, provider, txDefaults, logDecodeDependencies);
    contractInstance.constructorArgs = [_bZxProtocol,
_legacyVault
];
    ...
    for (const link of Object.values(links)) {
      for (const libraryName of Object.keys(link)) {
        if (!libraryAddresses[libraryName]) {
          ...
          await DAppHelperContract._deployLibrariesAsync(
            libraryArtifact,
            libraryArtifacts,
            web3Wrapper,
            txDefaults,
  ...
  public getFunctionSignature(methodName: string): string {
    ...
    const methodAbi = DAppHelperContract.ABI()[index] as MethodAbi; // tslint:disable-line:no-unnecessary-type-assertion
...
> {
    ...
    return {
      ...
      getABIEncodedTransactionData(): string {
        return self._strictEncodeArguments(functionSignature, [usdTokenAddress.toLowerCase(),
      tokens,
      sourceAmounts
      ]);
      },
...
> {
    const self = this as any as DAppHelperContract;
    const functionSignature = 'legacyVault()';
...
> {
    const self = this as any as DAppHelperContract;
      assert.isString('_bZxProtocol', _bZxProtocol);
      assert.isString('_legacyVault', _legacyVault);
    ...
    return {
      ...
      getABIEncodedTransactionData(): string {
        return self._strictEncodeArguments(functionSignature, [_bZxProtocol.toLowerCase(),
      _legacyVault.toLowerCase()
      ]);
      },
    }
  };
  ...
  constructor(
    ...
    txDefaults?: Partial<TxData>,
    logDecodeDependencies?: { [contractName: string]: ContractAbi },
    deployedBytecode: string | undefined = DAppHelperContract.deployedBytecode,
  ) {
    super('DAppHelper', DAppHelperContract.ABI(), address, supportedProvider, txDefaults, logDecodeDependencies, deployedBytecode);
    classUtils.bindAll(this, ['_abiEncoderByFunctionSignature', 'address', '_web3Wrapper']);
DAppHelperContract.ABI().forEach((item, index) => {
