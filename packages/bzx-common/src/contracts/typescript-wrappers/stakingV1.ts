...
public static contractName = 'StakingV1';
  private readonly _methodABIIndex: { [name: string]: number } = {};
private readonly _subscriptionManager: SubscriptionManager<StakingV1EventArgs, StakingV1Events>;
public static async deployFrom0xArtifactAsync(
    artifact: ContractArtifact | SimpleContractArtifact,
    supportedProvider: SupportedProvider,
    txDefaults: Partial<TxData>,
    logDecodeDependencies: { [contractName: string]: (ContractArtifact | SimpleContractArtifact) },
  ): Promise<StakingV1Contract> {
    ...
    return StakingV1Contract.deployAsync(bytecode, abi, provider, txDefaults, logDecodeDependenciesAbiOnly, );
  }
  ...
  public static async deployWithLibrariesFrom0xArtifactAsync(
    ...
    const libraryAddresses = await StakingV1Contract._deployLibrariesAsync(
      artifact,
      libraryArtifacts,
  ...
  public static async deployAsync(
    ...
    logUtils.log(`StakingV1 successfully deployed at ${txReceipt.contractAddress}`);
    const contractInstance = new StakingV1Contract(txReceipt.contractAddress as string, provider, txDefaults, logDecodeDependencies);
    ...
    for (const link of Object.values(links)) {
      for (const libraryName of Object.keys(link)) {
        if (!libraryAddresses[libraryName]) {
          ...
          await StakingV1Contract._deployLibrariesAsync(
            libraryArtifact,
            libraryArtifacts,
            web3Wrapper,
            txDefaults,
  ...
  public getFunctionSignature(methodName: string): string {
    ...
    const methodAbi = StakingV1Contract.ABI()[index] as MethodAbi; // tslint:disable-line:no-unnecessary-type-assertion
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'BZRXWeightStored()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'LPTokenWeightStored()';
...
> {
    const self = this as any as StakingV1Contract;
      ...
      assert.isArray('bzrxAmounts', bzrxAmounts);
      assert.isArray('stableCoinAmounts', stableCoinAmounts);
    const functionSignature = 'addDirectRewards(address[],uint256[],uint256[])';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isBigNumber('newBZRX', newBZRX);
      assert.isBigNumber('newStableCoin', newStableCoin);
    const functionSignature = 'addRewards(uint256,uint256)';
    ...
    return {
      ...
      getABIEncodedTransactionData(): string {
        return self._strictEncodeArguments(functionSignature, [newBZRX,
      newStableCoin
      ]);
      },
    }
...
> {
  ...
  public balanceOfByAssets(
        account: string,
  ): ContractFunctionObj<[BigNumber, BigNumber, BigNumber, BigNumber]
> {
    const self = this as any as StakingV1Contract;
      assert.isString('account', account);
...
> {
    const self = this as any as StakingV1Contract;
      assert.isString('account', account);
    const functionSignature = 'balanceOfStored(address)';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'bzrxPerTokenStored()';
...
> {
    ...
    const functionSignature = 'bzrxRewards(address)';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isString('index_0', index_0);
    const functionSignature = 'bzrxRewardsPerTokenPaid(address)';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isString('index_0', index_0);
    const functionSignature = 'bzrxVesting(address)';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'callerRewardDivisor()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'claim3Crv()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'claimBzrx()';
...
> {
    ...
    const functionSignature = 'currentFeeTokens(uint256)';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'curve3Crv()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'curve3pool()';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isString('account', account);
    const functionSignature = 'delegateBalanceOf(address)';
...
> {
    ...
    const functionSignature = 'delegatedPerToken(address,address)';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'fundsWallet()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'getVariableWeights()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'iBZRXWeightStored()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'lastRewardsAddTime()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'maxCurveDisagreement()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'maxUniswapDisagreement()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'rewardPercent()';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isBigNumber('_callerRewardDivisor', _callerRewardDivisor);
    const functionSignature = 'setCallerRewardDivisor(uint256)';
    ...
    return {
      ...
      getABIEncodedTransactionData(): string {
        return self._strictEncodeArguments(functionSignature, [_callerRewardDivisor
      ]);
      },
    }
  };
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'setCurveApproval()';
...
> {
    ...
    const functionSignature = 'setFeeTokens(address[])';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isString('_fundsWallet', _fundsWallet);
    const functionSignature = 'setFundsWallet(address)';
    ...
    return {
      ...
      getABIEncodedTransactionData(): string {
        return self._strictEncodeArguments(functionSignature, [_fundsWallet.toLowerCase()
      ]);
      },
    }
  };
...
> {
    const self = this as any as StakingV1Contract;
      assert.isBigNumber('_maxCurveDisagreement', _maxCurveDisagreement);
    const functionSignature = 'setMaxCurveDisagreement(uint256)';
    ...
    return {
      ...
      getABIEncodedTransactionData(): string {
        return self._strictEncodeArguments(functionSignature, [_maxCurveDisagreement
      ]);
      },
    }
  };
...
> {
    const self = this as any as StakingV1Contract;
      assert.isBigNumber('_maxUniswapDisagreement', _maxUniswapDisagreement);
    const functionSignature = 'setMaxUniswapDisagreement(uint256)';
    ...
    return {
      ...
      getABIEncodedTransactionData(): string {
        return self._strictEncodeArguments(functionSignature, [_maxUniswapDisagreement
      ]);
      },
    }
  };
...
> {
    ...
    const functionSignature = 'setPaths(address[][])';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isBigNumber('_rewardPercent', _rewardPercent);
    const functionSignature = 'setRewardPercent(uint256)';
    ...
    return {
      ...
      getABIEncodedTransactionData(): string {
        return self._strictEncodeArguments(functionSignature, [_rewardPercent
      ]);
      },
    }
  };
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'stableCoinPerTokenStored()';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isString('index_0', index_0);
    const functionSignature = 'stableCoinRewards(address)';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isString('index_0', index_0);
    const functionSignature = 'stableCoinRewardsPerTokenPaid(address)';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isString('index_0', index_0);
    const functionSignature = 'stableCoinVesting(address)';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isString('index_0', index_0);
    const functionSignature = 'stakingRewards(address)';
...
> {
    ...
    const functionSignature = 'swapPaths(address,uint256)';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'sweepFees()';
...
> {
    ...
    const functionSignature = 'sweepFeesByAsset(address[])';
    ...
    return {
      ...
      getABIEncodedTransactionData(): string {
        return self._strictEncodeArguments(functionSignature, [assets
      ]);
      },
    }
  };
...
> {
    ...
    const functionSignature = 'togglePause(bool)';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'totalSupplyStored()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'uniswapRouter()';
...
> {
    const self = this as any as StakingV1Contract;
    const functionSignature = 'vBZRXWeightStored()';
...
> {
    const self = this as any as StakingV1Contract;
      assert.isBigNumber('tokenBalance', tokenBalance);
      assert.isBigNumber('lastUpdate', lastUpdate);
      assert.isBigNumber('vestingEndTime', vestingEndTime);
    const functionSignature = 'vestedBalanceForAmount(uint256,uint256,uint256)';
    ...
    return {
      ...
      getABIEncodedTransactionData(): string {
        return self._strictEncodeArguments(functionSignature, [tokenBalance,
      lastUpdate,
      vestingEndTime
      ]);
      },
...
> {
  ...
  /**
    * Subscribe to an event type emitted by the StakingV1 contract.
    * @param eventName The StakingV1 contract event you would like to subscribe to.
    ...
    */
  public subscribe<ArgsType extends StakingV1EventArgs>(
    eventName: StakingV1Events,
    indexFilterValues: IndexedFilterValues,
    callback: EventCallback<ArgsType>,
    isVerbose: boolean = false,
    blockPollingIntervalMs?: number,
  ): string {
    assert.doesBelongToStringEnum('eventName', eventName, StakingV1Events);
  ...
  /**
    ...
    */
  public async getLogsAsync<ArgsType extends StakingV1EventArgs>(
    eventName: StakingV1Events,
  ...
  constructor(
    ...
    txDefaults?: Partial<TxData>,
    logDecodeDependencies?: { [contractName: string]: ContractAbi },
    deployedBytecode: string | undefined = StakingV1Contract.deployedBytecode,
  ) {
    super('StakingV1', StakingV1Contract.ABI(), address, supportedProvider, txDefaults, logDecodeDependencies, deployedBytecode);
    classUtils.bindAll(this, ['_abiEncoderByFunctionSignature', 'address', '_web3Wrapper']);
this._subscriptionManager = new SubscriptionManager<StakingV1EventArgs, StakingV1Events>(
      StakingV1Contract.ABI(),
      this._web3Wrapper,
    );
StakingV1Contract.ABI().forEach((item, index) => {
