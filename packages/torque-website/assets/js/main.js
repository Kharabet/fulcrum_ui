...
window.addEventListener('load', function() {
  ...
  if (formLoan) formLoan.addEventListener('input', onLoanInputChange, false)
  ...
  itemForm.forEach((item) => {
    ...
    item
      .querySelectorAll('.li-options')
      .forEach((option) => option.addEventListener('click', onItemFormLiClick))
  })
...
modal.onclick = function(event) {
  ...
  for (let i = 0; i < listSelect.length; i++) {
    if (event.target !== listSelect) listSelect[i].style.display = 'none'
  }
...
async function collateralEstimate(loanAsset, collateralAsset, amount) {
  ...
  return !responseJson.success ? errorLargeLoan : parseFloat(responseJson.data.depositAmount)
}
async function onLoanInputChange(e) {
  ...
  inputLoanValue > 0 ? form.classList.add('active') : form.classList.remove('active')
...
async function updateCollateralInput() {
  ...
  if (inputLoanValue === 0 || inputLoan.value.length === 0) {
    if (inputCollateral.classList.contains('error')) inputCollateral.classList.remove('error')
    inputCollateral.value = 0
  }
  if (inputLoanValue > 0) {
    const collateralEstimateValue = await collateralEstimate(
      loanAsset,
      collateralAsset,
      inputLoanValue
    )
...
function onStyledSelectClick(event) {
  ...
  const collateralAsset = document.querySelector('.item-form.collateral .select-styled').dataset
    .asset
  for (let i = 0; i < options.length; i++) {
    if (
      options[i].dataset.asset &&
      (options[i].dataset.asset === loanAsset || options[i].dataset.asset === collateralAsset)
    )
...
async function onItemFormLiClick(event) {
  ...
  if (itemForm.classList.contains('loan')) {
    ...
    if (window.borrowAPR && window.borrowAPR[asset])
      aprComponent.querySelector('.apr-value').textContent = parseFloat(
        window.borrowAPR[asset]
      ).toFixed(2)
    setTimeout(function() {
      loader.classList.remove('visible')
    }, 500)
...
async function getTorqueBorrowApr() {
  ...
  !responseJson.success ? console.error(responseJson.message) : renderBorrowApr(responseJson.data)
